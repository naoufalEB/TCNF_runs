...
def get_test_dataset(args, test_batch_size):
    ...
    """
    test_set = BMSequence(data_path=args.data_path, split=args.test_split, dim_x = args.effective_shape)
...
def get_dataset(args):
    ...
    """
    train_set = BMSequence(data_path=args.data_path, split="train", dim_x = args.input_size)
    val_set = BMSequence(data_path=args.data_path, split="val", dim_x = args.input_size)
...
class BMSequence(data.dataset.Dataset):
    ...
    def __init__(self, data_path, split="train", dim_x = 1):
    ...
    def __getitem__(self, index):
        ...
        #else:
        #    padded_values = torch.zeros(self.max_length, self.dim_x) + 100
        padded_values = torch.zeros(self.max_length, self.dim_x) + 100    
        
        masks = torch.ByteTensor(self.max_length).zero_()
        #masks = torch.ByteTensor(self.max_length, item_values.shape[1]).zero_()
        #masks[:item_len,:] = 1
        ...
        padded_variance = padded_variance.unsqueeze(1)
        # if self.dim_x > 1 :
        #     padded_times = padded_times.expand(padded_times.shape[0], self.dim_x)
        #     padded_variance = padded_variance.expand(padded_variance.shape[0], self.dim_x)
        # else:
        #     padded_values_return = padded_values
